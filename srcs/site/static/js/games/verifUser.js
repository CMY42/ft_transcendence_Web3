document.addEventListener('DOMContentLoaded', () => {
	...
	const submitLogin = async () => {
		...
		const loginErrorMessage = document.getElementById('loginErrorMessage'); // Sélecteur du message d'erreur
	
		loginErrorMessage.classList.add('d-none'); // Masque le message d'erreur au début de chaque tentative
		...
		if (!email || !password) {
			...
			// Affiche le message d'erreur dans l'élément d'alerte
			loginErrorMessage.textContent = enterEmailPasswordMessage;
		...
		try {
			...
			if (!response.ok) {
				...
				const errorText = errorData?.error || "Unknown email or password";
				
				// Affiche le message d'erreur de l'API
				loginErrorMessage.textContent = errorText;
				loginErrorMessage.classList.remove('d-none');
			...
			if (verifiedUsers.has(userData.username)) {
				...
				loginErrorMessage.textContent = userAlreadyLoggedInMessage;
				loginErrorMessage.classList.remove('d-none');
				return;
			}
