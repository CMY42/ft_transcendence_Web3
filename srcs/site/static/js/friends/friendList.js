...
export async function initFriendList() {
	...
	try {
						...
						<div class="col-md-8">
							<div class="card-body">
								...
								<p class="card-text user-joined-date">
									<span>Ami depuis :</span> <span class="timestamp">${friendEntry.timestamp}</span>
								</p>
							</div>
						</div>
		...
		showMessage('Une erreur est survenue lors de la récupération des amis.', 'danger');
	}
}
	...
	matches.forEach((match, index) => {
		...
		item.innerHTML = `
			...
			<div class="text-muted">
				<span>Joué le</span> <span class="match-date">${formatDate(match.date_played)}</span>
			</div>
...
async function removeFriend(friendId) {
    ...
    try {
        ...
        showMessage('Une erreur est survenue lors de la suppression de l\'ami.', 'danger');
    }
}
