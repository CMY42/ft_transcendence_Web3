	...
	try {
		...
		sortedUsers.forEach((user) => {
					...
					<div class="col-md-8">
						<div class="card-body">
							...
							<p class="card-text user-joined-date">
								<span>Membre depuis :</span> <span class="user-date-joined">${user.date_joined}</span>
							</p>
						</div>
					</div>
			...
			if (sentRequestIds.has(user.id)) {
				createActionButton(actionContainer, 'Annuler', ['btn', 'btn-danger', 'me-5', 'custom-size-btn', 'd-flex', 'justify-content-center', 'align-items-center'], () => cancelFriendRequest(user.id));
			} else if (receivedRequestIds.has(user.id)) {
				createActionButton(actionContainer, 'Accepter', ['btn', 'btn-success', 'mb-2', 'me-5', 'custom-size-btn', 'd-flex', 'justify-content-center', 'align-items-center'], () => acceptFriendRequest(user.id));
				createActionButton(actionContainer, 'Refuser', ['btn', 'btn-danger', 'me-5', 'custom-size-btn', 'd-flex', 'justify-content-center', 'align-items-center'], () => declineFriendRequest(user.id));
			} else {
				createActionButton(actionContainer, 'Ajouter', ['btn', 'btn-primary', 'me-5', 'custom-size-btn', 'd-flex', 'justify-content-center', 'align-items-center'], () => sendFriendRequest(user.id));
			}
		});
	} catch (error) {
...
async function sendFriendRequest(userId) {
	await handleFriendRequest('POST', 'send-friend-request', userId, 'Demande d\'ami envoyée avec succès !');
}

async function acceptFriendRequest(userId) {
	await handleFriendRequest('PATCH', 'accept-friend-request', userId, 'Demande d\'ami acceptée avec succès.');
}

async function declineFriendRequest(userId) {
	await handleFriendRequest('DELETE', 'decline-friend-request', userId, 'Demande d\'ami refusée avec succès.');
}

async function cancelFriendRequest(userId) {
	await handleFriendRequest('DELETE', 'cancel-friend-request', userId, 'Demande d\'ami annulée avec succès.');
}
...
async function handleFriendRequest(method, endpoint, userId, successMessage) {
    ...
    try {
        ...
        showMessage('Une erreur est survenue lors de l\'opération.', 'danger');
    }
}
